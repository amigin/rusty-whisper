...
pub struct Tokenizer {
    bpe: CoreBPE,
    pub lang2token: HashMap<String, u32>,
}

impl Tokenizer {
    pub fn new(vocab_path: &str) -> Tokenizer {
        let mut file = File::open(vocab_path).expect("Could not open file");
        ...
        let n_vocab = encoder.len() as Rank;
        ...
        let lang2token: HashMap<String, u32> = langs
            .iter()
            .enumerate()
            .map(|(index, pairs)| {
                (
                    String::from(pairs.0),
                    bpe.encode_with_special_tokens("<|startoftranscript|>")[0] + index as u32,
                )
            })
            .collect();
